 ...
 * - Varios vasos en una misma sala (#room=...)
 * - Estado + historial por vaso
 * - Sincronización Supabase si hay claves .env (VITE_SUPABASE_URL / VITE_SUPABASE_ANON_KEY)
 */

// ---------- Utilidades ----------

/** Mide el ancho disponible del contenedor (sustituye a ResponsiveContainer). */
...
/** Gráfico estable con Recharts (sin ResponsiveContainer, sin animación). */
function StableRechart({
  vaso,
  data,
  height = 220,
}: {
  vaso: Vaso;
  data: { dateISO: string; level: number }[];
  height?: number;
}) {
  const { ref, width } = useMeasuredWidth();

  return (
    <div ref={ref} className="h-56 w-full rounded-2xl border bg-white p-2">
      {width > 0 && data.length > 0 ? (
        ...
        <div className="h-[220px] flex items-center justify-center text-sm text-slate-500">
          {data.length === 0 ? "Aún no hay datos para mostrar." : "Preparando gráfico..."}
        </div>
      )}
    </div>
  );
...
// UID con fallback (algunos navegadores no tienen crypto.randomUUID)
const uid = () =>
...
function migrateIfNeeded(raw: any): State {
  ...
  if (raw && typeof raw === "object" && "capacity" in raw && "threshold" in raw) {
    ...
    return {
      vasos: { [me.id]: me, [other1.id]: other1, [other2.id]: other2 },
      order: [me.id, other1.id, other2.id],
      createdAtISO: todayISO(),
    };
  }
...
export default function VasoEmocionalApp() {
  ...
  const pushToCloudDebounced = useDebounced(async (snapshot: State) => {
    ...
    const { error } = await supabase
      .from("rooms")
      .upsert({ id: roomId, data: snapshot, updated_at: new Date().toISOString() });
    if (error) {
      setSaveStatus("error");
  ...
  // Guardado best-effort al cerrar/ocultar
  useEffect(() => {
    if (!supabase) return;
    const handler = () => {
      void supabase.from("rooms").upsert({ id: roomId, data: state, updated_at: new Date().toISOString() });
    };
    window.addEventListener("beforeunload", handler);
    const vis = () => document.visibilityState === "hidden" && handler();
          ...
          <div className="flex items-center gap-2 text-sm text-slate-600">
            {supabase ? (
              <span className="inline-flex items-center gap-1">
                <Wifi className={`h-4 w-4 ${online ? "opacity-100" : "opacity-40"}`} />{" "}
            ...
            <Button variant="outline" className="rounded-2xl" onClick={() => pushToCloudImmediate(state)}>
              Guardar ahora
            </Button>
            <Button
              variant="secondary"
              className="rounded-2xl inline-flex gap-2"
              onClick={() => navigator.clipboard.writeText(location.href).then(() => alert("Enlace copiado"))}
            >
...
function VasoCard({
  ...
  const chartData = useMemo(() => {
    ...
    return Array.from(dedup, ([dateISO, level]) => ({ dateISO, level })).sort((a, b) =>
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="relative w-40 h-64 rounded-3xl border-4 border-slate-300 bg-white overflow-hidden">
              <div
                className="absolute left-0 right-0 border-t-4 border-rose-400/80"
                style={{ bottom: `${(vaso.threshold / vaso.capacity) * 100}%` }}
              />
              <div
                className={`absolute bottom-0 left-0 right-0 transition-all duration-700 ${
                  inRedZone ? "bg-rose-200" : "bg-sky-200"
                }`}
            ...
            <div className="text-center">
              ...
              <div className="text-xs text-slate-600">
                Nivel: {vaso.level} / {vaso.capacity} gotas
              </div>
              {inRedZone && (
          ...
          <div className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <Input
                id={`label-${vaso.id}`}
                placeholder="p.ej., discusión por tareas"
                value={label}
              ...
              <div className="flex flex-wrap gap-2">
                <Button onClick={() => onAddEvent(label, drops)} className="rounded-2xl">
                  Añadir
                </Button>
                <Button variant="secondary" onClick={() => quickAdd("small")} className="rounded-2xl">
                  + pequeño (+2)
                </Button>
                <Button variant="secondary" onClick={() => quickAdd("medium")} className="rounded-2xl">
                  + mediano (+4)
                </Button>
                <Button variant="secondary" onClick={() => quickAdd("large")} className="rounded-2xl">
                  + grande (+8)
                </Button>
                <Button variant="ghost" onClick={onUndo} className="rounded-2xl inline-flex gap-2">
          ...
          <div className="rounded-2xl border bg-white p-4 shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4">
                ...
                <div className="flex items-center gap-3">
                  <Slider
                    ...
                    onValueChange={(v: number[]) =>
                      onChange({
                        capacity: v[0],
                        level: Math.min(v[0], vaso.level),
                        threshold: Math.min(v[0], vaso.threshold),
                      })
                    }
                  />
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <Slider
                    value={[vaso.threshold]}
                    min={0}
                    max={vaso.capacity}
                    step={1}
                    onValueChange={(v: number[]) => onChange({ threshold: v[0] })}
                  />
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <Slider
                    value={[vaso.evaporationPerDay]}
                    min={0}
                    max={10}
                    step={1}
                    onValueChange={(v: number[]) => onChange({ evaporationPerDay: v[0] })}
                  />
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <Slider
                    ...
                    max={vaso.capacity}
                    step={1}
                    onValueChange={(v: number[]) => onChange({ level: v[0] })}
                  />
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Switch
                    id={`auto-${vaso.id}`}
                    checked={vaso.autoDailyTick}
                    onCheckedChange={(val: boolean) => onChange({ autoDailyTick: Boolean(val) })}
                  />
                  <Label htmlFor={`auto-${vaso.id}`}>Evaporación automática</Label>
                </div>
                <Button
                  variant="outline"
                  onClick={() => onChange({ ...defaultVaso(vaso.name), id: vaso.id })}
                  className="rounded-2xl inline-flex gap-2"
                >
          ...
          <div>
            ...
            <StableRechart vaso={vaso} data={chartData} height={220} />
          </div>
